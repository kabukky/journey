...
func UpdateUser(id int64, name []byte, slug string, email []byte, image []byte, cover []byte, bio []byte, website []byte, location []byte, twitter []byte, facebook []byte, updated_at time.Time, updated_by int64) error {
	writeDB, err := readDB.Begin()
	if err != nil {
		writeDB.Rollback()
		return err
	}
	_, err = writeDB.Exec(stmtUpdateUser, name, slug, email, image, cover, bio, website, location, twitter, facebook, updated_at, updated_by, id)
	if err != nil {
		writeDB.Rollback()
		return err
	}
