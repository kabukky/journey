...
const stmtRetrievePostsForIndex = "SELECT id, uuid, title, slug, markdown, html, featured, page, status, image, author_id, published_at FROM posts WHERE page = 0 AND status = 'published' ORDER BY published_at DESC, id DESC LIMIT ? OFFSET ?"
